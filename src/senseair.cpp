...
void co2_setup()
{
    CO2Serial.begin(BAUDRATE, SERIAL_8N1, PIN_U1_RXD, PIN_U1_TXD);
}
...
void co2_requestValueAndStatus()
{
    ...
    clearResponse();
    if (!doRequest(CO2andStatusreq, 8, 13))
    {
        return;
    };
    co2_status = response[3] * 256 + response[4];
...
bool co2_clearBackgroundCalibrationAck()
{
    ...
    clearResponse();
    if (!doRequest(CalibrationReq1ClearAck, 8, 8))
    {
        return false;
    };
...
bool co2_startBackgroundCalibration()
{
    ...
    clearResponse();
    if (!doRequest(CalibrationReq2StartCalibration, 8, 8))
    {
        return false;
    };
    // Slave reply should be the same as the request (see documentation)
...
bool co2_checkBackgroundCalibrationAck()
{
    ...
    clearResponse();
    if (!doRequest(CalibrationReq3ReadAck, 8, 7))
    {
        return false;
    };
